...
def apply_threshold(model, threshold):
    with torch.no_grad():
        for param in model.parameters():
            param.data = torch.where(param.abs() < threshold, torch.tensor(0.0), param.data)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--clip_grad_norm", type=float, default=1.0, help="clip grad norm")
    parser.add_argument(
        "--checkpoint_every",
        type=int,
        default=100,
    ...
    def eval(net, loader):
        ...
        for X, Y, is_input in loader:
            ...
            with torch.no_grad():
                ...
                mask = 1 - is_input.reshape(-1)  # each element of mask indicates 0 as input and 1 as target to predict
            correct += (mask * (out.argmax(dim=-1) == Y)).sum().item()
            total += mask.sum().item()

            correct_input += ((1 - mask) * (out.argmax(dim=-1) == Y)).sum().item()
    ...
    if args.model == 'akorn':
        print(f"n: {args.N}, ch: {args.ch}, L: {args.L}, T: {args.T}, type of J: {args.J}")
        net = SudokuAKOrN(
            n=args.N,
            ch=args.ch,
            L=args.L,
        ...
        net = SudokuTransformer(ch=args.ch, 
                                blocks=args.L, 
                                heads=args.heads,
                                mlp_dim=args.ch * 2,
                                R=args.R,
    ...
    for epoch in range(args.epochs):
        ...
        for X, Y, is_input in tqdm.tqdm(trainloader):
            ...
            out = net(X, is_input)
            mask = 1 - is_input # each element of mask indicates 0 as input and 1 as target to predict
        ...
        if (epoch + 1) % args.eval_freq == 0:
            ...
            acc, input_acc, stats, stats_input  = eval(net, testloader)
            ...
            # EMA evals
            acc, input_acc, stats, stats_input  = eval(ema.ema_model, testloader)
