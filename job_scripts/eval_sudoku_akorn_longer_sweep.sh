...
for result_dir in results/sudoku_sweep_altered_loss_*; do
    if [ -d "$result_dir" ]; then
        ...
        run_id="${name##*_}"
        wandb_dir=$(find wandb -maxdepth 1 -type d -name "run-*-${run_id}" -print -quit)
        ...
        if [ -n "$wandb_dir" ] && [ -d "$wandb_dir" ]; then
            ...
            if [ -f "$config_file" ]; then
                L=$(grep -- "--L=" "$config_file" | head -n1 | cut -d'=' -f2)
                N=$(grep -- "--N=" "$config_file" | head -n1 | cut -d'=' -f2)
                T=$(grep -- "--T=" "$config_file" | head -n1 | cut -d'=' -f2)
                ch=$(grep -- "--ch=" "$config_file" | head -n1 | cut -d'=' -f2)
                loss=$(grep -- "--loss=" "$config_file" | head -n1 | cut -d'=' -f2)
