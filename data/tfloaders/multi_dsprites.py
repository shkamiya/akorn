...
MAX_NUM_ENTITIES = {"binarized": 4, "colored_on_grayscale": 6, "colored_on_colored": 5}
BYTE_FEATURES = ["mask", "image"]


def feature_descriptions(max_num_entities, is_grayscale=False):
    """Create a dictionary desc   ribing the dataset features.
...
def _decode(example_proto, features):
    ...
    for k in BYTE_FEATURES:
        single_example[k] = tf.squeeze(
            tf.decode_raw(single_example[k], tf.uint8), axis=-1
        )
...
def dataset(
    tfrecords_path, dataset_variant, read_buffer_size=None, map_parallel_calls=None
):
    ...
    if dataset_variant not in MAX_NUM_ENTITIES:
        raise ValueError(
            "Invalid `dataset_variant` provided. The supported values"
    ...
    raw_dataset = tf.data.TFRecordDataset(
        tfrecords_path, compression_type=COMPRESSION_TYPE, buffer_size=read_buffer_size
    )
    features = feature_descriptions(max_num_entities, is_grayscale)
    partial_decode_fn = functools.partial(_decode, features=features)
    return raw_dataset.map(partial_decode_fn, num_parallel_calls=map_parallel_calls)
